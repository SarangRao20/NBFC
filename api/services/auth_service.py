...
class AuthService:
    ...
    async def login_with_password(self, phone: str, password: str) -> Dict[str, Any]:
        """Login with phone and password using DB-first lookup with safe fallback."""
        phone = normalize_phone(phone)
        if len(phone) != 10:
        ...
        try:
            ...
            print(f"⚠️ DB lookup failed during login, falling back to mock users: {db_err}")

        # 2) Fallback: customers.json (for legacy records) + best-effort DB backfill
        if not user:
            customers = load_mock_customers()
        ...
        return {
            ...
            "customer_data": session_data.get("customer_data", user)
        }
